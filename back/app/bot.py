...
from app.bot_time import setup_time_commands
...
class EntrepreneurBot(commands.Bot):
    ...
    async def setup_hook(self) -> None:
        ...
        try:
            # TODO・目標・時間報告の Cog を先に登録しておく。コマンドのギルド同期は
            # setup_competition_commands の最後でまとめて行われる。
            await setup_todo_commands(self)
            await setup_goal_commands(self)
            await setup_time_commands(self)
        except Exception:
            logger.exception("Failed to register todo/goal/time commands")
        try:
